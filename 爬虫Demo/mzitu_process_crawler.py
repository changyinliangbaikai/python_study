...
def mzitu_crawler(max_threads=10):
    ...
    def pageurl_crawler():
        while True:
            try:
                ...
                path = title.replace('?', '') ##测试过程中发现一个标题有问号
                mkdir(path)
                os.chdir('/home/testee/lix/mztu/' + path)
    ...
    def mkdir(path):
        path = path.strip()
        isExists = os.path.exists(os.path.join("/home/testee/lix/mztu/", path))
        if not isExists:
            print u'建了一个名字叫做', path, u'的文件夹！'
            os.makedirs(os.path.join("/home/testee/lix/mztu/", path))
            return True
        else:
    ...
    while threads or crawl_queue:
        ...
        while len(threads) < max_threads or crawl_queue.peek(): ##线程池中的线程少于max_threads 或者 crawl_qeue时
            thread = threading.Thread(target= ) ##创建线程
