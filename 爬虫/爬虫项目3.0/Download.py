...
class download():
    ...
    def get(self, url, timeout, proxy=None, num_retries=1): ##给函数一个默认参数proxy为空
        ...
        if proxy == None: ##当代理为空时，不使用代理获取response（别忘了response啥哦！之前说过了！！）
            try:
                ...
                if response.status_code == 200:
                    ...
                    IP = ''.join(str(random.choice(self.__get_iplist())).strip()) ##下面有解释哦
                    proxy = {'http': IP}
            ...
            try:
                IP = ''.join(str(random.choice(self.__get_iplist())).strip()) ##将从self.__get_iplist()中获取的字符串处理成我们需要的格式（处理了些什么自己看哦，这是基础呢）
                ...
                if response.status_code == 200:
                    ...
                    if num_retries > 0:
                        time.sleep(1)
                        IP = ''.join(str(random.choice(self.__get_iplist())).strip())
                        proxy = {'http': IP}
