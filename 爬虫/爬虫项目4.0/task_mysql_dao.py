...
def get_db():
    db = MySQLdb.connect(host="127.0.0.1", user="root", passwd="g6s8m3t7s", db="spider", charset="utf8")
...
def task_peek():
    db,cursor = get_db()
    sql = "select id,url,type,depth,keyword from task_list where status = 1 limit 10"
    result = ""
    try:
        cursor.execute(sql)
...
def add_one_task_result(task_id,url,content):
    find_sql = "select id from task_result where url = %s and content = %s and taskId = %s"
    sql = "insert into task_result (taskId,url,content) values (%s,%s,%s)"
    db, cursor = get_db()
    try:
        cursor.execute(find_sql,(url,content,task_id,))
